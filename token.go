...
func parseColMetadata72(r *tdsBuffer, s *tdsSession) (columns []columnStruct) {
	...
	for i := range columns {
		...
		typeInfo := readTypeInfo(r, baseTi.TypeId, column.cryptoMeta, s)
		...
		if column.isEncrypted() && s.alwaysEncrypted {
			...
			cryptoMeta := parseCryptoMetadata(r, cekTable, s)
...
func parseCryptoMetadata(r *tdsBuffer, cekTable *cekTable, s *tdsSession) cryptoMetadata {
	ordinal := uint16(0)
	if cekTable != nil {
		ordinal = r.uint16()
	}
	...
	ti := readTypeInfo(r, typeInfo.TypeId, nil, s)
...
func parseReturnValue(r *tdsBuffer, s *tdsSession) (nv namedValue) {
	...
	*/
	...
	if s.alwaysEncrypted && (ti.Flags&fEncrypted) == fEncrypted {
		cm := parseCryptoMetadata(r, nil, s) // CryptoMetadata
		cryptoMetadata = &cm
	}

	ti2 := readTypeInfo(r, ti.TypeId, cryptoMetadata, s)
